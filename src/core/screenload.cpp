...
ScreenLoad* ScreenLoad::instance()
{
    static ScreenLoad c;
    return &c;
}

CaptureWidget* ScreenLoad::gui(const CaptureRequest& req)
{
    ...
    // to switch back to the ScreenLoad Desktop manually. It is not obvious and a
...
void ScreenLoad::screen(CaptureRequest req, const int screenNumber)
{
    if (!resolveAnyConfigErrors()) {
        return;
    }
...
void ScreenLoad::full(const CaptureRequest& req)
{
    if (!resolveAnyConfigErrors()) {
        return;
    }
    ...
    if (ok) {
        QRect selection; // `screenload full` does not support --selection
...
QVersionNumber ScreenLoad::getVersion()
{
    return QVersionNumber::fromString(
      QStringLiteral(APP_VERSION).replace("v", ""));
}

void ScreenLoad::setOrigin(Origin origin)
{
    m_origin = origin;
}

ScreenLoad::Origin ScreenLoad::origin()
{
    return m_origin;
}
 ...
 */
bool ScreenLoad::resolveAnyConfigErrors()
{
    bool resolved = true;
...
void ScreenLoad::requestCapture(const CaptureRequest& request)
{
    if (!resolveAnyConfigErrors()) {
        return;
    }
...
void ScreenLoad::exportCapture(QPixmap capture,
                              QRect& selection,
                              const CaptureRequest& req)
{
    ...
    if (tasks & CR::COPY) {
        ScreenLoadDaemon::copyToClipboard(capture);
    }

    if (tasks & CR::PIN) {
        ScreenLoadDaemon::createPin(capture, selection);
...
void ScreenLoad::setExternalWidget(bool b)
{
    m_haveExternalWidget = b;
}
bool ScreenLoad::haveExternalWidget()
{
    return m_haveExternalWidget;
}

// STATIC ATTRIBUTES
ScreenLoad::Origin ScreenLoad::m_origin = ScreenLoad::DAEMON;
